# -*- coding: utf-8 -*-
"""Google Search Console 周报（ClearPNG 版）：拉取最近 7 天的自然搜索查询表现。

站点通过环境变量 GSC_SITE 配置，默认 https://png.my99ai.com/。
认证二选一（未配置时优雅跳过）：
  1) OAuth 刷新令牌（推荐）：GSC_OAUTH_CLIENT_ID / GSC_OAUTH_CLIENT_SECRET /
     GSC_OAUTH_REFRESH_TOKEN
  2) 服务账号 JSON：GSC_SERVICE_ACCOUNT_JSON
...
SITE = os.getenv('GSC_SITE', 'https://png.my99ai.com')
HOST = urlparse(SITE).netloc
SITES = [SITE.rstrip('/') + '/', SITE.rstrip('/'), f'sc-domain:{HOST}']
...
def main():
    ...
    for site in SITES:
        ...
        if rows is not None:
            ...
            break
    if rows is None:
        raise SystemExit('GSC API 访问失败：请确认站点已在 Search Console 验证且账号有权限')

    clicks = sum(r['clicks'] for r in rows)
    imps = sum(r['impressions'] for r in rows)
    lines = [
        f'## SEO 周报 - ClearPNG（{start} ~ {end}）',
        '',
